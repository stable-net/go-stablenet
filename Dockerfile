...
# Build Gstable in a stock Go builder container
...
ADD . /go-ethereum
RUN cd /go-ethereum && go run build/ci.go install -static ./cmd/gstable

# Pull Gstable into a second stage deploy alpine container
FROM alpine:latest

RUN apk add --no-cache ca-certificates
COPY --from=builder /go-ethereum/build/bin/gstable /usr/local/bin/
