...
func TestAccountUpdate(t *testing.T) {
	t.Parallel()
	datadir := tmpDatadirWithKeystore(t)
	geth := runGstable(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	t.Parallel()
	geth := runGstable(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer geth.ExpectExit()
	geth.Expect(`
!! Unsupported terminal, password will be echoed.
...
func TestUnlockFlag(t *testing.T) {
	t.Parallel()
	geth := runMinimalGstable(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
