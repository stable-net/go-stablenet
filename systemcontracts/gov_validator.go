...
// upgradeValidator performs a migration-style upgrade for GovValidator.
// Only parameters present in param are written. Missing keys are skipped.
func upgradeValidator(govValidatorAddress common.Address, param map[string]string) ([]params.StateParam, error) {
	sp, err := upgradeBase(govValidatorAddress, param)
	if err != nil {
		return nil, err
	}

	// gasTip: only if provided
	if gasTipStr, ok := param[GOV_VALIDATOR_PARAM_GAS_TIP]; ok {
		gasTip, parseOk := new(big.Int).SetString(gasTipStr, 10)
		if !parseOk {
			return nil, fmt.Errorf("`systemContracts.govValidator.params.gasTip`: invalid value: %s", gasTipStr)
	...
	// validators + blsPublicKeys: only if provided
	if valStr, ok := param[GOV_VALIDATOR_PARAM_VALIDATORS]; ok {
		if _, ok2 := param[GOV_VALIDATOR_PARAM_BLS_KEYS]; !ok2 {
			return nil, fmt.Errorf("`systemContracts.govValidator.params`: missing parameter: %s", GOV_VALIDATOR_PARAM_BLS_KEYS)
		}
		...
		for i, val := range validators {
			...
			member := common.HexToAddress(memberAddresses[i])
			sp = append(sp,
				params.StateParam{Address: govValidatorAddress, Key: CalculateMappingSlot(indexSlot, val), Value: common.BigToHash(newLength)},
				params.StateParam{Address: govValidatorAddress, Key: CalculateDynamicSlot(valueSlot, new(big.Int).SetUint64(currentIdx)), Value: common.BytesToHash(val.Bytes())},
				params.StateParam{Address: govValidatorAddress, Key: CalculateMappingSlot(common.HexToHash(SLOT_VALIDATOR_validatorToOperator), val), Value: common.BytesToHash(member.Bytes())},
				params.StateParam{Address: govValidatorAddress, Key: CalculateMappingSlot(common.HexToHash(SLOT_VALIDATOR_operatorToValidator), member), Value: common.BytesToHash(val.Bytes())},
			)
			sp = append(sp, MakeMultipleParam(govValidatorAddress, CalculateMappingSlot(common.HexToHash(SLOT_VALIDATOR_validatorToBlsKey), val), VarLenBytesToMultipleHash(blsKeys[i]))...)
			sp = append(sp, params.StateParam{
				Address: govValidatorAddress,
				Key:     CalculateMappingSlot(common.HexToHash(SLOT_VALIDATOR_blsKeyToValidator), &blsWrapper{blsKey: blsKeys[i]}),
				Value:   common.BytesToHash(val.Bytes()),
			})
			duplicated[val] = struct{}{}
			currentIdx++
		...
		if newLength.Sign() > 0 {
			sp = append(sp, params.StateParam{Address: govValidatorAddress, Key: valueSlot, Value: common.BigToHash(newLength)})
		}
	}
