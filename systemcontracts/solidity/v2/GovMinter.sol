...
contract GovMinter is GovBase {
    ...
    // Refundable balance for non-executed burn proposals (Pull withdrawal pattern)
    ...
    /// @notice Emitted when burn deposit is moved to refundable balance on non-executed proposal finalization
    event BurnDepositRefunded(uint256 indexed proposalId, address indexed requester, uint256 amount);

    /// @notice Emitted when a user claims their refundable burn deposit
    event BurnRefundClaimed(address indexed requester, uint256 amount);
    ...
    /// @notice Claim refundable burn deposit (Pull withdrawal pattern)
    /// @dev No access control — removed members must also be able to claim refunds
    /// @dev Uses CEI pattern: zero balance before external call
    function claimBurnRefund() external {
        ...
        emit BurnRefundClaimed(msg.sender, amount);
    }
    ...
    /// @dev Clean up burn deposit when proposal reaches terminal state
    /// @notice Moves burnBalance to refundableBalance for non-executed proposals
    /// @param proposalId The proposal ID to clean up
    ///
    /// @custom:idempotency Safe to call multiple times (amount == 0 early return)
    /// @custom:security No external calls — safe within GovBase finalization hook
    function _cleanupBurnDeposit(uint256 proposalId) internal {
        BurnProposalData memory data = burnProposals[proposalId];
        if (data.amount == 0) return; // idempotent: already cleaned or not a burn proposal

        // For Executed proposals, _safeBurn already decremented burnBalance on success.
        // For Failed proposals, _safeBurn's try-catch returned false without modifying burnBalance.
        // For Cancelled/Expired/Rejected, _safeBurn was never called.
        ProposalStatus status = proposals[proposalId].status;
        if (status != ProposalStatus.Executed) {
            // Move from burnBalance to refundableBalance (no external calls)
            burnBalance[data.requester] -= data.amount;
            refundableBalance[data.requester] += data.amount;
            emit BurnDepositRefunded(proposalId, data.requester, data.amount);
        }

        // Clean up proposal data in all cases
        delete burnProposals[proposalId];
    }

    function _onProposalFinalized(uint256 proposalId) internal override {
        _cleanupMintReservation(proposalId);
        _cleanupBurnDeposit(proposalId);
    }
