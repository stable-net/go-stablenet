...
var (
	...
	authorizedReceipts = Receipts{
		...
		&Receipt{
			...
			TxHash:            txs[6].Hash(),
			GasUsed:           7,
			EffectiveGasPrice: big.NewInt(2600),
			BlobGasUsed:       3 * params.BlobTxBlobGasPerBlob,
			BlobGasPrice:      big.NewInt(920),
			BlockHash:         blockHash,
			BlockNumber:       blockNumber,
...
func TestDeriveFieldsAuthorizedAccount(t *testing.T) {
	...
	err := Receipts(derivedReceipts).DeriveFields(params.TestChainConfig, blockHash, blockNumber.Uint64(), blockTime, basefee, nil, blobGasPrice, txs)
	if err != nil {
		t.Fatalf("DeriveFields(...) = %v, want <nil>", err)
	}
