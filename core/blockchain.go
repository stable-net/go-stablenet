...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	if chainConfig.AnzeonEnabled() {
		if err := chainConfig.Anzeon.CheckValidity(); err != nil {
	...
	// If Gstable is initialized with an external ancient store, re-initialize the
...
// it's not checked in the Gstable side anymore.
...
func (bc *BlockChain) collectLogs(b *types.Block, removed bool) []*types.Log {
	...
	if err := receipts.DeriveFields(bc.chainConfig, b.Hash(), b.NumberU64(), b.Time(), b.BaseFee(), b.Header().MinerTip(), blobGasPrice, b.Transactions()); err != nil {
